...
def generate_content_from_topic(topic: str) -> CardNewsData:
    """
    Generates single-narrative complete (기승전결 4-act) Instagram Card News.
    Strictly binds to the user's requested animal/topic.
    Uses verified Google News RSS if available, otherwise uses Gemini AI to retrieve a verified real-world news story.
    ...
    if news_item:
        ...
        news_context = f"""
    [실시간 검증된 해외 언론 보도 정보]
    - 기사 제목: {news_item["title"]}
    - 언론사 출처: {source_name}
    - 원문 링크: {source_url}
    - 기사 요약: {news_item["snippet"]}
        """
    else:
        source_name = "Global News Archive"
        source_url = "https://news.google.com"
        is_fallback = True
        news_context = f"""
    [주제 기반 실화 사건 자율 탐색 모드]
    - 사용자 요청 주제: "{topic}"
    - 지침: 전 세계 주요 언론(BBC, CNN, Daily Mail, 워싱턴포스트, 로이터 등)에 실제로 보도되어 큰 화제가 되었던 **"{topic}" 관련 실제 엽기/감동/놀라운 단일 실화 사건**을 팩트 기반으로 선정하여 작성하세요.
        """
    ...
    prompt = f"""
    [🚨 절대 준수 필수 제약 조건]
    1. 사용자가 요청한 핵심 주제/동물은 **"{topic}"** 입니다.
    2. 카드뉴스의 주인공과 모든 서사는 **반드시 "{topic}"에 관한 단 하나의 실제 사건 스토리**여야 합니다. 절대로 다른 동물(캥거루, 여우, 쥐 등)로 이탈하지 마세요!
    3. 하나의 사건에 대해 **처음부터 결말까지 완벽한 기승전결(4-Act Narrative Storytelling)** 구조로 총 6장의 인스타그램 카드뉴스를 작성하세요.
    ...
    1. **단일 사건 완결형 4단계 서사 구조**:
       ...
       - **슬라이드 3 (전: 반전 & 클라이맥스)**: 의외의 범인/행동이나 기발한 반전이 드러나는 피크 절정 순간.
...
{dynamic_learning_prompt}
    ...
    {{
      ...
      "cover": {{
        ...
        "image_prompt": "English image prompt for cover slide with {topic}"
      }},
      "body_slides": [
        {{
      ...
      "hashtags": ["#세계동물뉴스", "#{topic}", "#동물실화", "#카드뉴스"]
    }}
    """
